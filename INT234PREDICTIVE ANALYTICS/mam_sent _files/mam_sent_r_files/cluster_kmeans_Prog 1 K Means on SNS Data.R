...
teens$no_gender <- ifelse(is.na(teens$gender), 
                          1, 0)
#if gender is NA, it will assign 1 to no_gender 
#variable
table(teens$gender, useNA = "ifany")
table(teens$female, useNA = "ifany")
#compare original gender column with newly 
#created female 
...
#remove missing data once to find mean for 
#average age for graduation year
aggregate(data = teens, age ~ gradyear, 
          mean, na.rm = TRUE) 
...
ave_age <- ave(teens$age, teens$gradyear, FUN =
                 function(x) 
                   mean(x, na.rm = TRUE))
ave_age
#use ave function to calculate the mean of
#all the subgroups available
...
interests_z <- as.data.frame(lapply
                      (interests, scale))
#scale to rescale features to get mean of 0 
...
#model performance
teen_clusters$size 
#show the cluster size of 5 clusters. so many
#teenagers have same interest so to verify find 
#the centroid
