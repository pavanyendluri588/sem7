...
View(concrete)
str(concrete)
#total 9 variables are there. one is 'strength' dependent on all the other 8 variables.
...
library(neuralnet)
#intsall the package neuralnet for neural network implementation and load it to the program
...
#It returns a list with two components: $neurons, which stores theneurons for each layer in the network, and 
